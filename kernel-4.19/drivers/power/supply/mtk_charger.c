...
int wait_for_temp_stable = -1;
bool g_into_warm_over_cv;
EXPORT_SYMBOL(g_into_warm_over_cv);
...
void do_sw_jeita_state_machine(struct mtk_charger *info)
{
	...
	if (info->battery_temp > info->data.temp_t4_thres) {
		...
		if ((sw_jeita->sm == TEMP_ABOVE_T4)
			...
			sw_jeita->charging = false;
		} else if (sw_jeita->sm == TEMP_T2_TO_T3 && info->battery_temp <= info->data.temp_t3_thres_plus_x_degree) {
			chr_err("[SW_JEITA] Battery Temperature between %d and %d,not change to warm temperature charging yet!!\n",
		...
		} else if (sw_jeita->sm == TEMP_T2_TO_T3 && info->battery_temp >= info->data.temp_t2_thres_minus_x_degree) {
			chr_err("[SW_JEITA] Battery Temperature between %d and %d,not change to cool temperature charging yet!!\n",
					info->data.temp_t2_thres_minus_x_degree,
					info->data.temp_t2_thres);
		} else {
			chr_err("[SW_JEITA] Battery Temperature between %d and %d !!\n",
...
static void temp_stable_handler(struct work_struct *work)
{
	struct mtk_charger *info= container_of(work, struct mtk_charger, temp_stable_work.work);
	static int count;
	if (wait_for_temp_stable == 1 && count >9){
		count = 0;
		wait_for_temp_stable = 0;
		return;
	} else if (wait_for_temp_stable == 2 && count >4){
		count = 0;
		wait_for_temp_stable = 0;
		return;
	}

	if (wait_for_temp_stable == 1 && count < 10){
		count++;
		schedule_delayed_work(&info->temp_stable_work, msecs_to_jiffies(60000));
	} else if (wait_for_temp_stable == 2 && count < 6){
		count++;
		schedule_delayed_work(&info->temp_stable_work, msecs_to_jiffies(60000));
	}

	pr_err("temp_stable_handler---wait_status:%d count:%d\n",wait_for_temp_stable ,count);
}
...
static void charger_check_status(struct mtk_charger *info)
{
	...
	bool charge_status = true;
	int charger_type;
	static int count = 0;

	charger_type = get_charger_type(info);
	if (charger_type == POWER_SUPPLY_TYPE_UNKNOWN) {
		g_into_warm_over_cv = false;
		count = 0;
		return;
	}
	else if (charger_type == POWER_SUPPLY_TYPE_USB_FLOAT) {
		if (info->charger_thread_polling == true && count < 10) {
			info->polling_interval = 1;  //maybe hub connected, check it more frequently
	...
	if (temperature <= -20) {
		chr_err("Battery Temperature Under -20, NTC maybe disconnected, power off!");
		kernel_power_off();
	}
	...
	if (info->enable_sw_jeita == true) {
		...
		if (bat_psy) {
			gm = bat_psy->drv_data;
			charger_dev_get_charging_status(info->chg1_dev, &charge_status);
			chr_err("%s: state_machine = %d, suspend_flag = %d, charge_status = %d\n", __func__, (info->sw_jeita).sm, info->chg1_dev->suspend_flag, charge_status);
			if ((info->sw_jeita).sm == TEMP_T3_TO_T4 && info->chg1_dev->suspend_flag == 0 && charge_status == false)
				g_into_warm_over_cv = true;
			else
				g_into_warm_over_cv = false;
		}
				...
				int chr_type)
{
	...
	{
		wait_for_temp_stable = 2;
		schedule_delayed_work(&info->temp_stable_work, msecs_to_jiffies(60000));
	} else {
		wait_for_temp_stable = 0;
	}

	chr_err("mtk_is_charger_on plug in, type:%d temp:%d temp_stable:%d\n", chr_type, temp, wait_for_temp_stable);
...
static void kpoc_power_off_check(struct mtk_charger *info)
{
	...
	if (boot_mode == 8 || boot_mode == 9) {
		...
		if (vbus < 2500 ) {
			...
			mdelay(2000); //wait 5 seconds for plug out animation done
			kernel_power_off();
		}
	}
}
					       ...
					       enum power_supply_property psp)
{
	...
	case POWER_SUPPLY_PROP_INPUT_CURRENT_LIMIT:
		return 1;
	case POWER_SUPPLY_PROP_DIS_CHARGING_CURRENT:
		return 1;
	default:
		return 0;
	}
...
static enum power_supply_property charger_psy_properties[] = {
	...
	POWER_SUPPLY_PROP_WAIT_TEMP_STATUS,
	POWER_SUPPLY_PROP_DIS_CHARGING_CURRENT,
};
	...
	enum power_supply_property psp, union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_VOLTAGE_NOW:
		...
		break;
	case POWER_SUPPLY_PROP_WAIT_TEMP_STATUS:
		val->intval = wait_for_temp_stable;
		break;
	...
	case POWER_SUPPLY_PROP_VOLTAGE_BOOT:
		...
		break;
	case POWER_SUPPLY_PROP_DIS_CHARGING_CURRENT:
		val->intval = info->dis_battery_charging;
		pr_err("get battery charging:%d\n",val->intval);
		break;
	default:
		return -EINVAL;
	}
			...
			const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_INPUT_CURRENT_LIMIT:
		...
		break;
	case POWER_SUPPLY_PROP_DIS_CHARGING_CURRENT:
		info->dis_battery_charging = val->intval;
		pr_err("set charging:%d\n",info->dis_battery_charging);
		break;
	default:
		return -EINVAL;
	}
...
static int mtk_charger_probe(struct platform_device *pdev)
{
	...
	INIT_DELAYED_WORK(&info->temp_stable_work, temp_stable_handler);
