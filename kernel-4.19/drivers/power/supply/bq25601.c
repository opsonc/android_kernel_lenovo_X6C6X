...
static const char *charger_name = "Unknown";
...
unsigned int bq25601_get_eta_part_id(void)
{
	unsigned int ret = 0;
	unsigned char val = 0;
	...
	ret = bq25601_read_interface((unsigned char) (bq25601_CON11),
				     (&val),
				     (unsigned char) (CON11_ETA_PART_ID_MASK),
				     (unsigned char) (CON11_ETA_PART_ID_SHIFT)
				    );
	return val;
}
				   ...
				   bool en)
{
	...
	pr_info("enable state : %d, suspend_flag = %d\n", en, chg_dev->suspend_flag);
	if (en && !chg_dev->suspend_flag) {
				     ...
				     u32 current_value)
{
	...
	if(current_value > 2000000){
		pr_info("&&&& input_current_value = %d !!!\n", current_value);
		current_value = 2000000;
		pr_info("&&&& max input current = %d !!!\n", current_value);
	}
	current_value /= 10;
...
static int bq25601_get_all_charging_status(struct charger_device *chg_dev,
                                       bool *is_charging)
{
	unsigned int ret_val;
...
static int bq25601_get_ibus(struct charger_device *chg_dev,
			       u32 *ibus)
{
	unsigned int ret_val = 0;
        *ibus = 2000000;
...
static int bq25601_set_ship_mode(struct charger_device *chg_dev, bool en, bool delay)
{
	bq25601_set_batfet_delay(delay ? 1 : 0);
	bq25601_set_batfet_disable(en ? 1 : 0);
	return 0;
}

static int bq25601_get_charger_name(struct charger_device *chg_dev, const char **name)
{
	*name = charger_name;
	return 0;
}
...
static unsigned int charging_hw_init(void)
{
	...
	bq25601_set_batfet_reset_enable(0); /*Disable reset function*/
...
static struct charger_ops bq25601_chg_ops = {
	...
	.get_charging_status = bq25601_get_all_charging_status,
	.get_ibus_adc = bq25601_get_ibus,
	...
	/* ship mode */
	.set_ship_mode = bq25601_set_ship_mode,

	/* charger name */
	.get_charger_name = bq25601_get_charger_name,
};
				...
				const struct i2c_device_id *id)
{
	...
	if(!g_bq25601_hw_exist)
	{
		...
		pr_err("###[%s:line=%d] BQ25601 is not exist!\n",__func__,__LINE__);
		return -ENODEV;
	}
	...
	if (bq25601_get_eta_part_id())
		charger_name = "YT_ETA6953";
	else
		charger_name = "TI_BQ25601";
