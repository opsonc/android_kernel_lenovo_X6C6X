...
static int himax_get_max_dc(void)
{
	...
	I("%s: tmp_data[0]=%x,tmp_data[1]=%x,tmp_data[2]=%x,tmp_data[3]=%x\n", __func__,
			tmp_data[0], tmp_data[1], tmp_data[2], tmp_data[3]);

	dc_max = tmp_data[3] << 24 | tmp_data[2] << 16 | tmp_data[1] << 8 | tmp_data[0];
	I("%s: dc max = %d\n", __func__, dc_max);
	return dc_max;
}
...
*/
	for (i = 0; i < HX_CRITERIA_ITEM - 1; i++) {
		if (g_test_item_flag[i] == 1) {
			I("=====i=%d==========%s: ======lgc=%d=\n",i, __func__,((ret & (1 << (i + ERR_SFT))) == (1 << (i + ERR_SFT))));
		//	seq_printf(s, "%s%d%s",	(0 == result_count)? "":"-",result_count,((ret & (1 << (i + ERR_SFT))) == (1 << (i + ERR_SFT))) ? "F":"P");
			I("=====i=%s==========%s: ======lgc=%s=\n",i, __func__,s);
			if((ret & (1 << (i + ERR_SFT))) == (1 << (i + ERR_SFT)))
			 b_result &= 0;
			result_count++;
		}
	}
	I("=====b_result=%d==========%s: ======lgc=%s===\n",b_result, __func__,s);
	seq_printf(s, "result=%s",b_result ? "1":"0");
