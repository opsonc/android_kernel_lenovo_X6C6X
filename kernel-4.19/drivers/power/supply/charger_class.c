...
int charger_dev_get_charging_status(struct charger_device *charger_dev, bool *en)
{
	if (charger_dev != NULL && charger_dev->ops != NULL &&
		charger_dev->ops->get_charging_status)
		return charger_dev->ops->get_charging_status(charger_dev, en);

	return -ENOTSUPP;
}
EXPORT_SYMBOL(charger_dev_get_charging_status);

int charger_dev_set_ship_mode(struct charger_device *charger_dev, bool en, bool delay)
{
	if (charger_dev != NULL && charger_dev->ops != NULL &&
		charger_dev->ops->set_ship_mode)
		return charger_dev->ops->set_ship_mode(charger_dev, en, delay);

	return -ENOTSUPP;
}
EXPORT_SYMBOL(charger_dev_set_ship_mode);

int charger_dev_get_charger_name(struct charger_device *charger_dev, const char **name)
{
	if (charger_dev != NULL && charger_dev->ops != NULL &&
		charger_dev->ops->get_charger_name)
		return charger_dev->ops->get_charger_name(charger_dev, name);

	return -ENOTSUPP;
}
EXPORT_SYMBOL(charger_dev_get_charger_name);
