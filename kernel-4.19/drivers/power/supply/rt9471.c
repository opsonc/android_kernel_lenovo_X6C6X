...
static int rt_device_id = 0;

static int rt9471_read_device(void *client, u8 addr, u8 len, u8 *dst)
{
	//pr_err("###[%s:line=%d] addr=%d, len=%d,p_dst=0x%p\n",__func__,__LINE__,addr,len,dst);
	return i2c_smbus_read_i2c_block_data(client, addr, len, dst);
}

static int rt9471_write_device(void *client, u8 addr, u8 len,
			       const u8 *src)
{
					 ...
					 u8 *data)
{
	...
	//pr_err("###[%s:line=%d]In: cmd=0x%x, p_data=0x%p\n",__func__,__LINE__,data);
	...
	//pr_err("###[%s:line=%d] ret=%d, regval=0x%xx\n",__func__,__LINE__,ret,regval);
			       ...
			       bool *is_one)
{
	...
	//pr_err("###[%s:line=%d]In: cmd=0x%x, is_one=0x%p, p_regval=0x%p\n",__func__,__LINE__,cmd,is_one,&regval);

	ret = rt9471_i2c_read_byte(chip, cmd, &regval);
	//pr_err("###[%s:line=%d] cmd=0x%x,regval=0x%x\n",__func__,__LINE__,cmd,regval);
	if (ret < 0) {
		*is_one = false;
		return ret;
	}
	...
	*is_one = (regval ? true : false);
	...
	//pr_err("###[%s:line=%d] *is_one=%d\n",__func__,__LINE__,*is_one);
				  ...
				  u8 mask)
{
	...
	//pr_err("###[%s:line=%d]In: ret=%d,cmd=0x%x,regval=0x%x\n",__func__,__LINE__,ret,cmd,regval);
	if (ret < 0)
		goto out;
...
static int __rt9471_get_mivr(struct rt9471_chip *chip, u32 *mivr)
{
	...
	//pr_err("###[%s:line=%d]ret=%d,regval=0x%x\n",__func__,__LINE__,ret,regval);
	if (ret < 0)
		return ret;
	...
	*mivr = rt9471_closest_value(RT9471_MIVR_MIN, RT9471_MIVR_MAX,
	...
	//pr_err("###[%s:line=%d]mivr=%d\n",__func__,__LINE__,mivr);
...
static int __rt9471_get_aicr(struct rt9471_chip *chip, u32 *aicr)
{
	...
	*aicr = rt9471_closest_value(RT9471_AICR_MIN, RT9471_AICR_MAX,
	...
	//pr_err("###[%s:line=%d]aicr=%d\n",__func__,__LINE__,*aicr);
...
static int __rt9471_get_ieoc(struct rt9471_chip *chip, u32 *ieoc)
{
	...
	*ieoc = rt9471_closest_value(RT9471_IEOC_MIN, RT9471_IEOC_MAX,
	...
	//pr_err("###[%s:line=%d]ieoc=%d\n",__func__,__LINE__,*ieoc);
	return ret;
}
...
static int __rt9471_enable_shipmode_delay(struct rt9471_chip *chip, bool en)
{
	dev_info(chip->dev, "%s en = %d\n", __func__, en);
	return (en ? rt9471_set_bit : rt9471_clr_bit)
		(chip, RT9471_REG_FUNCTION, BIT(6));
}
...
static int __rt9471_dump_registers(struct rt9471_chip *chip)
{
	...
	rt9471_i2c_read_byte(chip, RT9471_REG_REGU, &sv);
	dev_info(chip->dev, "%s RT9471_REG_REGU(0x6) = 0x%02X\n", __func__, sv);

	rt9471_i2c_read_byte(chip, RT9471_REG_FUNCTION, &fuc);
	dev_info(chip->dev, "%s RT9471_REG_FUNCTION(0x2) = 0x%02X\n", __func__, fuc);
	return 0;
}
...
static bool rt9471_check_devinfo(struct rt9471_chip *chip)
{
	...
	pr_err("###[%s:line=%d] RT9471_REG_INFO=0x%x\n",__func__,__LINE__,ret);
	if (ret < 0) {
	...
	rt_device_id = chip->dev_id;
	pr_err("###[%s:line=%d] chip->dev_id=0x%x\n",__func__,__LINE__,chip->dev_id);
	switch (chip->dev_id) {
	case RT9470_DEVID:
	case RT9470D_DEVID:
	case RT9471_DEVID:
...
*/	
	ret = __rt9471_enable_chg(chip, en && !chg_dev->suspend_flag);
	if (ret < 0) {
...
*/
	...
	ret = __rt9471_enable_hz(chip, en && chg_dev->suspend_flag);
	if (ret < 0)
		dev_notice(chip->dev, "%s en hz fail(%d)\n",
				      __func__, ret);
	return ret;
...
static int rt9471_set_mivr(struct charger_device *chg_dev, u32 uV)
{
	...
	//pr_err("###[%s:line=%d]In:uV=%d\n",__func__,__LINE__,uV);
...
static int rt9471_set_aicr(struct charger_device *chg_dev, u32 uA)
{
	...
	//pr_err("###[%s:line=%d]In:uA=0x%x\n",__func__,__LINE__,uA);
...
static int rt9471_get_all_charging_status(struct charger_device *chg_dev,
                                       bool *is_charging)
{
	int ret = 0;
	u8 regval = 0;
	...
	ret = rt9471_i2c_read_byte(chip, RT9471_REG_STATUS, &regval);
	//pr_err("###[%s:line=%d] cmd = 0x%x,regval = 0x%x\n",__func__,__LINE__,RT9471_REG_STATUS,regval);
	if(ret < 0)
		return ret;
...
static int rt9471_get_charger_name(struct charger_device *chg_dev, const char **name)
{
	//struct rt9471_chip *chip = container_of(chg_dev, struct rt9471_chip, chg_dev); 	    
...
int rt9471_set_ship_mode(struct charger_device *chg_dev, bool en, bool delay)
{
	struct rt9471_chip *chip = dev_get_drvdata(&chg_dev->dev);

	__rt9471_enable_shipmode_delay(chip,!!delay);
	__rt9471_enable_shipmode(chip,en);
...
static struct charger_ops rt9471_chg_ops = {
	...
	.get_charging_status = rt9471_get_all_charging_status,

	.get_charger_name = rt9471_get_charger_name,
	
	.set_ship_mode = rt9471_set_ship_mode,
};
...
static int rt9471_regulator_is_enabled(struct regulator_dev *rdev)
{
	...
	//pr_err("###[%s:line=%d]en=%d\n",__func__,__LINE__,en);
	if (ret < 0)
		dev_notice(chip->dev, "%s: read is_chg_enabled fail\n",
								__func__);
	return en;
}
							...
							unsigned int sel)
{
	...
	//pr_err("###[%s:line=%d]In: sel=%d\n",__func__,__LINE__,sel);
	return rt9471_i2c_update_bits(chip, desc->vsel_reg,
				sel << RT9471_OTGCV_SHIFT, desc->vsel_mask);
}
...
static int rt9471_regulator_get_voltage_sel(struct regulator_dev *rdev)
{
	...
	//pr_err("###[%s:line=%d]In\n",__func__,__LINE__);
	ret = rt9471_i2c_read_byte(chip, desc->vsel_reg, &data);
	//pr_err("###[%s:line=%d]ret=%d,data=0x%x\n",__func__,__LINE__,ret,data);
	if (ret < 0)
		return ret;
	return (data & desc->vsel_mask) >> RT9471_OTGCV_SHIFT;
}
						...
						int min_uA, int max_uA)
{
	...
	//pr_err("###[%s:line=%d]In: min_uA=%d, max_uA=%d\n",__func__,__LINE__,min_uA,max_uA);
	...
	//return rt9471_set_boost_current_limit(chip->chg_dev, min_uA);
	return rt9471_set_boost_current_limit(chip->chg_dev, rt9471_otgcc[1]);
}


int rt9471_enable_regulator_otg(struct regulator_dev *rdev)
{
	int ret = 0;
	struct rt9471_chip *chip = rdev_get_drvdata(rdev);
	...
	ret = rt9471_enable_discharge(chip->chg_dev, false);
	if (ret < 0) {
		dev_notice(chip->dev, "%s: disable discharge fail\n", __func__);
		return ret;
	}

	return rt9471_enable_otg(chip->chg_dev, true);
}

int rt9471_disable_regulator_otg(struct regulator_dev *rdev)
{
	int ret = 0;
	struct rt9471_chip *chip = rdev_get_drvdata(rdev);
	...
	ret = rt9471_enable_otg(chip->chg_dev, false);
	if (ret < 0) {
		dev_notice(chip->dev, "%s: disable otg fail\n", __func__);
		return ret;
	}

	return rt9471_enable_discharge(chip->chg_dev, true);
}
...
static int rt9471_get_current_limit(struct regulator_dev *rdev)
{
	...
	ret = rt9471_i2c_read_byte(chip, RT9471_REG_OTGCFG, &val);
	//pr_err("###[%s:line=%d] ret=%d,val=0x%x\n",__func__,__LINE__,ret,val);
	if (ret < 0) {
		dev_notice(chip->dev, "%s: read otg_cc fail\n", __func__);
		return ret;
	}
	val = (val & RT9471_OTGCC_MASK) >> RT9471_OTGCC_SHIFT;
	//pr_err("###[%s:line=%d] val=0x%x,rt9471_otgcc[val]=%d\n",__func__,__LINE__,val,rt9471_otgcc[val]);
	return rt9471_otgcc[val];
}


 static const struct regulator_ops rt9471_chg_otg_ops = {
 	.enable = rt9471_enable_regulator_otg,
 	.disable = rt9471_disable_regulator_otg,
			...
			const struct i2c_device_id *id)
{
	...
	if (!rt9471_check_devinfo(chip)) {
		pr_err("###[%s:line=%d] RT9471/RT9471D/RT9470/RT9470D is not exist!\n",__func__,__LINE__);
		ret = -ENODEV;
		goto err_nodev;
	}
