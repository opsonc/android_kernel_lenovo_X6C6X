...
#define length	6
unsigned int Useful[length] = {0}; //通道电容值
unsigned int Average[length] = {0};//电容平均值
unsigned int DIFF[length] = {0};  //通道值与平均值的差值
unsigned int Offset[length] = {0}; //电容补偿值

unsigned int ch1_report = 0;
unsigned int ch3_report = 0;
unsigned int Sar_chip_id = 0;
unsigned int main_distance_val = 0;
unsigned int wifi_distance_val = 0;
...
static int sx933x_Hardware_Check(psx93XX_t this)
{
	...
	printk("--lyd_sar, sx933x_Hardware_Check....\n");
	...
	Sar_chip_id = failcode;

    dev_info(this->pdev, "[SX933x]: sx933x failcode = 0x%x\n",this->failStatusCode);
	printk("--lyd_sar, sx933x ID = 0x%x\n", failcode);
	printk("--lyd_sar, sx933x failStatusCode = 0x%x\n", this->failStatusCode);
        ...
        struct device_attribute *attr, const char *buf, size_t count)
{
    ...
    printk("--lyd_sar, manual_offset_calibration, val = %d\n", val);
...
static void read_rawData(psx93XX_t this)
{
    ...
    if(this)
    {
        for(csx = 0; csx < 5; csx++)
        {
            ...
            //dev_info(this->pdev, "[SX933x]: [CS: %d] Useful = %d Average = %d, DIFF = %d Offset = %d \n",
...
static ssize_t sx933x_reg_diff_show(struct device *dev,
                                    struct device_attribute *attr, char *buf)
{
    psx93XX_t this = dev_get_drvdata(dev);
    read_rawData(this);

    return sprintf(buf, "CH1 diff : %d; CH3 diff : %d;", DIFF[1], DIFF[3]);
}

static ssize_t sx933x_reg_useful_show(struct device *dev,
                                    struct device_attribute *attr, char *buf)
{
    psx93XX_t this = dev_get_drvdata(dev);
    read_rawData(this);

    return sprintf(buf, "CH0 Useful : %d; CH1 Useful : %d; CH3 Useful : %d; CH4 Useful : %d",
			            Useful[0], Useful[1], Useful[3], Useful[4]);
}

static ssize_t sx933x_IC_name_show(struct device *dev,
                                    struct device_attribute *attr, char *buf)
{
    //psx93XX_t this = dev_get_drvdata(dev);
	char IC_name[8] = "SX9338";

    return  sprintf(buf, "%s", IC_name);
}


static ssize_t sx933x_CH1_report_show(struct device *dev,
                                    struct device_attribute *attr, char *buf)
{
    return  sprintf(buf, "%d", ch1_report);
}

static ssize_t sx933x_CH3_report_show(struct device *dev,
                                    struct device_attribute *attr, char *buf)
{
    return  sprintf(buf, "%d", ch3_report);
}

static ssize_t sx933x_Sar_ID_show(struct device *dev,
                                    struct device_attribute *attr, char *buf)
{
    return  sprintf(buf, "0x%x", Sar_chip_id);
}

static ssize_t sx933x_main_distance_show(struct device *dev,
                                    struct device_attribute *attr, char *buf)
{
    return  sprintf(buf, "0x%x", main_distance_val);
}

static ssize_t sx933x_wifi_distance_show(struct device *dev,
                                    struct device_attribute *attr, char *buf)
{
    return  sprintf(buf, "0x%x", wifi_distance_val);
}
...
static DEVICE_ATTR(reg_diff, 0664, sx933x_reg_diff_show, NULL);
static DEVICE_ATTR(reg_useful, 0664, sx933x_reg_useful_show, NULL);
static DEVICE_ATTR(IC_name, 0664, sx933x_IC_name_show, NULL);

static DEVICE_ATTR(CH1_report, 0664, sx933x_CH1_report_show, NULL);
static DEVICE_ATTR(CH3_report, 0664, sx933x_CH3_report_show, NULL);

static DEVICE_ATTR(Sar_ID, 0664, sx933x_Sar_ID_show, NULL);
static DEVICE_ATTR(Sar_WIFI, 0664, sx933x_wifi_distance_show, NULL);
static DEVICE_ATTR(Sar_MAIN, 0664, sx933x_main_distance_show, NULL);
...
static struct attribute *sx933x_attributes[] =
{
    ...
    &dev_attr_reg_diff.attr,
    &dev_attr_reg_useful.attr,
    &dev_attr_IC_name.attr,
    &dev_attr_CH1_report.attr,
    &dev_attr_CH3_report.attr,
    &dev_attr_Sar_ID.attr,
    &dev_attr_Sar_WIFI.attr,
    &dev_attr_Sar_MAIN.attr,
    NULL,
};
...
static void touchProcess(psx93XX_t this)
{
    ...
    struct _buttonInfo *pCurrentButton_tmp  = NULL;

    if (this && (pDevice = this->pDevice))
    {
		printk("--lyd_sar, enter touch Process function.....\n");
        sx933x_i2c_read_16bit(this, SX933X_STAT0_REG, &i);
		printk("--lyd_sar, status reg values = %d\n", i);
        ...
        for (counter = 0; counter < numberOfButtons; counter++)
        {
            ...
            case IDLE: /* Button is not being touched! */
                if (((i & pCurrentButton->mask) == pCurrentButton->mask))
                {
                    ...
                    /* User pressed button */
                    dev_info(this->pdev, "lyd_sar ,[SX933x]:Button %d touched\n", counter);
                    if(counter == 1)
                    {
                        //report wifi F2
                        wifi_distance_val = pCurrentButton->keycode;
                        input_report_key(input, pCurrentButton->keycode, 1);
                        input_report_key(input, pCurrentButton->keycode, 0);
                        dev_info(this->pdev, "lyd_sar, [SX933x]:Button down %d report\n",counter);
                    
                    }
                    else if(counter == 3)
                    {
                        //report sar F4
                        main_distance_val = pCurrentButton->keycode;
                        input_report_key(input, pCurrentButton->keycode, 1);
                        input_report_key(input, pCurrentButton->keycode, 0);
                        dev_info(this->pdev, "lyd_sar, [SX933x]:Button down %d report\n",counter);
                    }
                ...
                {
                    dev_info(this->pdev, "lyd_sar, [SX933x]:Button %d already released.\n",counter);
                }
                break;
            case ACTIVE: /* Button is being touched! */
                if (((i & pCurrentButton->mask) != pCurrentButton->mask))
                {
                    ...
                    {
                        counter = 2;
                        pCurrentButton_tmp = &buttons[counter];
                        //sar report F6
                        wifi_distance_val = pCurrentButton_tmp->keycode;
                        input_report_key(input, pCurrentButton_tmp->keycode, 1);
                        input_report_key(input, pCurrentButton_tmp->keycode, 0);
                        dev_info(this->pdev, "lyd_sar, [SX933x]:Button up %d,wifi F3 report\n",counter);
                    }
                    else if(counter == 3)
                    {
                        //sar report f5
                        counter = 4;
                        pCurrentButton_tmp = &buttons[counter];
                        main_distance_val = pCurrentButton_tmp->keycode;
                        input_report_key(input, pCurrentButton_tmp->keycode, 1);
                        input_report_key(input, pCurrentButton_tmp->keycode, 0);
                        dev_info(this->pdev, "lyd_sar, [SX933x]:Button up %d report\n",counter);
                    }
                    else if (counter == 2 || counter == 4)
                    {
                    }
                    ...
                    {
                        dev_info(this->pdev, "lyd_sar, [SX933x]:Button %d released\n",counter);
                        input_report_key(input, pCurrentButton->keycode, 0);
                    }
                ...
                {
                    dev_info(this->pdev, "lyd_sar, [SX933x]:Button %d still touched.\n",counter);
                }
                break;
    ...
    if (work)
    {
        ...
        while((++counter) < MAX_NUM_STATUS_BITS)   /* counter start from MSB */
        {
            if (((status>>counter) & 0x01) && (this->statusFunc[counter]))
            {
                    ...
                    printk("--lyd_sar, status function counter = %d, status = %d\n", counter, status);
                #endif
...
void sx93XX_suspend(psx93XX_t this)
{
    if (this)
    {
	printk(KERN_ERR "yl  sx93XX_suspend\n");
...
void sx93XX_resume(psx93XX_t this)
{
    if (this)
    {
        printk(KERN_ERR "yl sx93XX_resume\n");
        //enable_irq(this->irq);
    }
}
