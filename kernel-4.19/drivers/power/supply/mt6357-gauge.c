	...
	struct mtk_gauge_sysfs_field_info *attr, int val)
{
	...
	pr_err("[%s] ui_soc=%d, spare3_reg=0x%x, valid:%d, new_spare3_reg:0x%x\n",
...
static int battery_voltage_get(struct mtk_gauge *gauge, int *val)
{
	int ret;
	...
	struct mtk_gauge_sysfs_field_info *attr, int *val)
{
	...
	*val = 0;
	...
	battery_voltage_get(gauge, &bat_volt);
