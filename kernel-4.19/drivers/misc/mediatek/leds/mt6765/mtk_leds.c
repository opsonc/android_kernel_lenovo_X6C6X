...
extern int lcd_bl_set_led_brightness_ktz8864a(int value);
extern int lcd_bl_dump_reg_ktz8864a(void);
struct cust_mt65xx_led *get_cust_led_dtsi(void)
{
	...
	printk("[%s]: hyper\n", __func__);
				pled_dtsi[i].data =
				    (long)lcd_bl_set_led_brightness_ktz8864a;
#endif /* CONFIG_BACKLIGHT_SUPPORT_LM3697 */
				pr_info
				    ("hyper kernel:the BL hw mode is LCM.\n");
				break;
			case MT65XX_LED_MODE_CUST_BLS_PWM:
				if(lcd_bl_dump_reg_ktz8864a())
				{
					pled_dtsi[i].data =
				    (long)lcd_bl_set_led_brightness_ktz8864a;
