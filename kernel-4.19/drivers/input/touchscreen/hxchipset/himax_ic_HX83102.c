...
{
  	if (!(strcmp(str, "hx83102p_fhd_lenovo_k10_330nit"))) {
  		is_hx_lcm = 1;
  		printk("Func:%s is_hx :%d", __func__, is_hx_lcm);
	} else if (!(strcmp(str, "hx83102p_fhd_lenovo_k10_400nit"))) {
		is_hx_lcm = 2;
		printk("Func:%s is_hx :%d", __func__, is_hx_lcm);
	} else if (!(strcmp(str, "nt36523_fhd_lenovo_k10_330nit"))){
		is_hx_lcm = 3;
		printk("Func:%s is_hx :%d", __func__, is_hx_lcm);
	} 
	else if( !(strcmp(str, "nt36523_fhd_lenovo_k10_400nit")) ) {
		is_hx_lcm = 4;
		printk("Func:%s is_hx :%d", __func__, is_hx_lcm);
	}
...
bool islcd_connect;
int __init islcd(char *str)
{
  	if (!(strcmp(str, "1"))) {
  		islcd_connect = true;
  		printk("Func:%s islcd_connect :%d", __func__, islcd_connect);
...
{
	int ret = 0;
  	printk("%s result  is_hx:%d",__func__,is_hx_lcm);
	if(3 == is_hx_lcm || 0 == islcd_connect || 4 == is_hx_lcm)
		return -1;
	I("%s\n", __func__);
