	...
	struct chg_limit_setting *setting)
{
	...
	if (info->dis_battery_charging == 1) {
		pdata->charging_current_limit = 0;
		is_basic = true;
		goto done;
	}

	pr_err("select_chaging:%d dis_batery:%d\n",pdata->charging_current_limit ,info->dis_battery_charging);
	...
	if (info->chr_type == POWER_SUPPLY_TYPE_USB) {
		...
		/* it can be larger */
		pdata->charging_current_limit = 1000 * 1000;  /* 1A */
		is_basic = true;
	...
	if (info->enable_sw_jeita) {
		if (IS_ENABLED(CONFIG_USBIF_COMPLIANCE)
			...
			if (info->sw_jeita.sm == TEMP_T1_TO_T2 || info->sw_jeita.sm == TEMP_T3_TO_T4) {
...
//		charger_dev_enable(info->chg1_dev, false);
// just stop the current into battery, do not shut usb current
		charger_dev_set_charging_current(info->chg1_dev, 0);
		charger_dev_do_event(info->chg1_dev, EVENT_DISCHARGE, 0);
	} else {
// later, do_algorithm will recover the charging current
//		charger_dev_enable(info->chg1_dev, true);
		charger_dev_do_event(info->chg1_dev, EVENT_RECHARGE, 0);
	}
