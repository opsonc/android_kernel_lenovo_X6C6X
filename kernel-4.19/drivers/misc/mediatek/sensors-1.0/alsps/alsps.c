...
int32_t last_prox_status = -1;
int32_t last_prox_data = 0;
...
int ps_data_report_t(int value, int status, int64_t time_stamp)
{
	...
	event.flush_action = DATA_ACTION;
	event.time_stamp = time_stamp;
	event.word[0] = (value & 0x00000001) + 1;
	event.word[1] = (value >> 1);
	event.status = status;

    if ((event.word[0] != last_prox_status) || (((event.word[1] - last_prox_data) >= 50) || ((last_prox_data - event.word[1]) >= 50))) {
        last_prox_status = event.word[0];
        last_prox_data = event.word[1];
        err = sensor_input_event(alsps_context_obj->ps_mdev.minor, &event);
        printk("lyd_ps, %d %d", last_prox_status, last_prox_data);
    }
	return err;
}
