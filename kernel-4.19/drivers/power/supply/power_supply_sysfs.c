...
static ssize_t power_supply_show_property(struct device *dev,
	...
	case POWER_SUPPLY_PROP_CHARGE_COUNTER_EXT:
		ret = sprintf(buf, "%lld\n", value.int64val);
		break;
	case POWER_SUPPLY_PROP_MODEL_NAME ... POWER_SUPPLY_PROP_CHARGE_IC:
		ret = sprintf(buf, "%s\n", value.strval);
		break;
	default:
		ret = sprintf(buf, "%d\n", value.intval);
...
static struct device_attribute power_supply_attrs[] = {
	...
	POWER_SUPPLY_ATTR(dis_charging_current),
	/* Capacity Estimation */
	...
	POWER_SUPPLY_ATTR(is_pd_port),
#endif
	POWER_SUPPLY_ATTR(is_float_type),
	...
	POWER_SUPPLY_ATTR(charge_ic),
	POWER_SUPPLY_ATTR(wait_temp_stable),
};
