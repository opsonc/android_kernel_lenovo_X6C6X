 ...
 * Filename:
 * ---------
 *	 ov5675_ofilm_front_mipiraw_Sensor.c
 *
 * Project:
 * --------
 *	 ALPS
...
#include "ov5675_ofilm_front_mipiraw_Sensor.h"
...
static imgsensor_info_struct imgsensor_info = {
	.sensor_id = OV5675_OFILM_FRONT_SENSOR_ID,	//record sensor id defined in Kd_imgsensor.h
...
static bool update_otp(void)
{
	...
	LOG_ERR("[ov5675] otp rg = %d, bg = %d,golden_rg = %d, golden_bg = %d\n", rg, bg, golden_rg, golden_bg);
	r_ratio = ((1024*golden_rg)/rg);
	b_ratio = ((1024*golden_bg)/bg);
	LOG_ERR("[ov5675] otp wb r_ratio = %d, b_ratio = %d\n", r_ratio, b_ratio);
	...
	if ((R_GAIN == 0) && (B_GAIN == 0) && (G_GAIN == 0)){
		...
		LOG_INF("[ov5675] otp1 wb R_GAIN = %d, B_GAIN = %d, G_GAIN = %d\n", R_GAIN, B_GAIN, G_GAIN);
	}
	...
	if ((R_GAIN != 0) && (B_GAIN != 0) && (G_GAIN != 0))
	{
		...
		LOG_INF("[ov5675] otp2 wb R_GAIN = %d, B_GAIN = %d, G_GAIN = %d\n", R_GAIN, B_GAIN, G_GAIN);
	}

	LOG_INF("[ov5675] otp3 wb R_GAIN = %d, B_GAIN = %d, G_GAIN = %d\n", R_GAIN, B_GAIN, G_GAIN);
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
	...
	//sensor have two i2c address 0x6c 0x6d & 0x21 0x20, we should detect i2c address the module used
...
static kal_uint32 open(void)
{
	...
	sensor_id = read_cmos_sensor_8(0x0005);
	LOG_ERR("fgj otp sensor id: 0x%x\n",sensor_id);
...
{
	pr_err("OV5675 streaming_enable(0=Sw Standby,1=streaming): %d\n",
		enable);
	if (enable) {
		/* MIPI// */
		write_cmos_sensor(0x0100, 0x01);
...
UINT32 OV5675_OFILM_FRONT_MIPI_RAW_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
