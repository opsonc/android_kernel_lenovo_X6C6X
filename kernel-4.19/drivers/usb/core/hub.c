...
static bool first_probe = 1;//if first probe it is root hub don't set hub_connected
unsigned int hub_connected = 0;
...
static void hub_release(struct kref *kref)
{
	struct usb_hub *hub = container_of(kref, struct usb_hub, kref);
        ...
        hub_connected = 0;
        printk("hub_release hub_connected=%d\n", hub_connected);
...
static int hub_probe(struct usb_interface *intf, const struct usb_device_id *id)
{
	...
	dev_info(&intf->dev, "USB hub found\n");
        if(!first_probe)
            hub_connected = 1;
        dev_info(&intf->dev, "hub_connected = %d, first_probe = %d\n", hub_connected, first_probe);
