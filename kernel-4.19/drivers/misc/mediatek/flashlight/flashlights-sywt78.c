					...
					const char *buf, size_t count)
{
	...
	if ((strcmp("0", buf) == 0) || (strcmp("0\x0a", buf) == 0)){
		PK_DBG(" sywt78_FLASH  0");
		sywt78_disable();
		sywt78_set_driver(0);
	} else {
		PK_DBG(" sywt78_FLASH  1");
		sywt78_set_driver(1);
		sywt78_set_level(0);
		sywt78_timeout_ms = 0;
		sywt78_enable();
...
static DEVICE_ATTR(sywt78_FLASH, 0664, att_show, att_store);
...
static int sywt78_probe(struct platform_device *pdev)
{
	...
	err = sysfs_create_file(&pdev->dev.kobj, &dev_attr_sywt78_FLASH.attr);
	if(err)
	{
		PK_ERR("Failed to sysfs_create_file:err=%d\n", err);
	}
