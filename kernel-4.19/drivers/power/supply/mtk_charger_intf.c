...
int get_charger_type(struct mtk_charger *info)
{
	...
	if (chg_psy == NULL || IS_ERR(chg_psy)) {
		...
		if (prop.intval == 0)
			...
			if (hub_connected) {
				...
				info->chr_type = prop2.intval;
			}
			else
				prop2.intval = POWER_SUPPLY_TYPE_USB_FLOAT;
		}
		power_supply_get_property(chg_psy, POWER_SUPPLY_PROP_IS_FLOAT_TYPE, &prop_float);
		if (prop2.intval == POWER_SUPPLY_TYPE_USB_FLOAT) {
			if (prop_float.intval != 1) {
				prop_float.intval = 1;
				power_supply_set_property(chg_psy, POWER_SUPPLY_PROP_IS_FLOAT_TYPE, &prop_float);
				power_supply_changed(chg_psy);
			}
		}
		else {
			if (prop_float.intval != 0) {
				prop_float.intval = 0;
				power_supply_set_property(chg_psy, POWER_SUPPLY_PROP_IS_FLOAT_TYPE, &prop_float);
				power_supply_changed(chg_psy);
			}
	...
	pr_notice("%s online:%d hub_connected:%u type:%d usb_type:%d\n", __func__,
		prop.intval,
		hub_connected,
		prop2.intval,
		prop3.intval);
