...
int ccci_get_opt_val(char *opt_name)
{
	...
	int ret;
        ...
        /* wifi sku will return 0, or it can not boot */
	if (NULL != strstr(saved_command_line, "androidboot.rsc=master_wifi") || NULL !=strstr(saved_command_line, "androidboot.rsc=nb_wifi"))
		return 0;
	...
	if ((md_support_val & MD_CAP_ENHANCE) == MD_CAP_ENHANCE) {
		if (md_support_val & (MD_CAP_FDD_LTE | MD_CAP_TDD_LTE)) {
			if ((md_support_val & (MD_CAP_TDS_CDMA | MD_CAP_WCDMA))
				...
				if (md_id == MD_SYS1) {
					if (NULL != strstr(saved_command_line, "androidboot.rsc=master_wifi") || NULL !=strstr(saved_command_line, "androidboot.rsc=nb_wifi"))
						return 0; /* wifi sku will return 0 */
					else
