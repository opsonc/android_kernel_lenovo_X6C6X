...
struct proc_dir_entry *himax_proc_himax_info_file;
uint8_t byte_length;
uint8_t reg_cmd[4];
uint8_t cfg_flag;
				...
				size_t len, loff_t *pos)
{
	...
	if (!HX_PROC_SEND_FLAG) {
		ret += snprintf(buf_tmp + ret, sizeof(buf_tmp) - ret,
				"BOE,%s,FW:0x%2.2X\n", private_ts->chip_name,ic_data->vendor_touch_cfg_ver);
...
static ssize_t himax_info_read(struct file *file, char *buf,
				size_t len, loff_t *pos)
{
	ssize_t ret = 0;
	...
	if (!HX_PROC_SEND_FLAG) {
		ret += snprintf(buf_tmp + ret, sizeof(buf_tmp) - ret,
				"BOE,%s,FW:0x%2.2X\n", private_ts->chip_name,ic_data->vendor_fw_ver);
...
static const struct file_operations himax_proc_himax_info_file_ops = {
	.owner = THIS_MODULE,
	.read = himax_info_read,
};
...
int himax_touch_proc_init(void)
{
	...
	himax_proc_himax_info_file = proc_create(HIMAX_PROC_HIMAX_INFO_FILE, 0444,
	  himax_touch_proc_dir, &himax_proc_himax_info_file_ops);
	if (himax_proc_himax_info_file == NULL) {
		E(" %s: proc himax_info file create failed!\n", __func__);
		goto fail_1;
	}
...
fail_6: remove_proc_entry(HIMAX_PROC_PANEL_DIRECTION_FILE, himax_touch_proc_dir_1);
...
void himax_touch_proc_deinit(void)
{
	...
	remove_proc_entry(HIMAX_PROC_HIMAX_INFO_FILE, himax_touch_proc_dir);
}
