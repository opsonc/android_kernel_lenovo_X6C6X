		...
		int len, uint8_t *data, uint8_t bank)
{
	...
	if (w_data == NULL) {
		E("%s w_data allocate chk buf failed\n", __func__);
		goto Write_FAIL0;
	}
	chk_data = kcalloc(data_len, sizeof(bool), GFP_KERNEL);
	if (chk_data == NULL) {
		E("%s chk_data allocate chk buf failed\n", __func__);
		goto Write_FAIL1;
	}
...
int himax_mcu_0f_operation_dirly(void)
{
	...
	if(panel_info == PANEL_BOE){
		...
		ret = request_firmware(&fw_entry, BOOT_UPGRADE_FWNAME_BOE,
		private_ts->dev);
	}else if(panel_info == PANEL_INX){
		I("%s, Entering,file name = %s\n", __func__, BOOT_UPGRADE_FWNAME_INX);
		ret = request_firmware(&fw_entry, BOOT_UPGRADE_FWNAME_INX,
		private_ts->dev);
	}else{
		I("%s, Entering,file name = %s\n", __func__, BOOT_UPGRADE_FWNAME_BOE);
		ret = request_firmware(&fw_entry, BOOT_UPGRADE_FWNAME_BOE,
...
void himax_mcu_0f_operation(struct work_struct *work)
{
	...
	if(panel_info == PANEL_BOE){
		I("%s, Entering,file name = %s\n", __func__, BOOT_UPGRADE_FWNAME_BOE);
		err = request_firmware(&fw_entry, BOOT_UPGRADE_FWNAME_BOE,
		private_ts->dev);
	}else if(panel_info == PANEL_INX){
		I("%s, Entering,file name = %s\n", __func__, BOOT_UPGRADE_FWNAME_INX);
		err = request_firmware(&fw_entry, BOOT_UPGRADE_FWNAME_INX,
		private_ts->dev);
	}else{
		I("%s, Entering,file name = %s\n", __func__, BOOT_UPGRADE_FWNAME_BOE);
		err = request_firmware(&fw_entry, BOOT_UPGRADE_FWNAME_BOE,
