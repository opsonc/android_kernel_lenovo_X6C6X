...
#ifdef CONFIG_BATTERY_GAUGE_MM8013C10
#include "mm8013c10.h"
#endif

extern bool g_into_warm_over_cv;
...
#ifdef CONFIG_BATTERY_GAUGE_MM8013C10
bool g_init_flag = true;
#define UISOC_ADD 10
#define UISOC_SUB 60
#define LOW_BATT_UISOC 10
static void update_soc_handler(struct work_struct *work)
{
	struct mtk_battery *gm;
	...
	int ret;
	int bat_volt;
	int real_soc, rtc_ui_soc;
	bool is_done = false;
	static int ui_soc;
	gm = container_of(work,
		struct mtk_battery, update_soc_work.work);
	get_monotonic_boottime(&now_time);
	real_soc = mm8013c10_get_bat_info(MM8013C10_BAT_SOC);
	charger_dev_is_charging_done(gm->charger_dev, &is_done);
	...
	if (g_init_flag == true) {
	    ...
	    if (rtc_ui_soc > 0 && rtc_ui_soc <= 100){
			pr_err("[update_soc_work]rtc_ui_soc is %d\n",rtc_ui_soc);
			ui_soc = rtc_ui_soc;
        } else {
			ui_soc = real_soc;
			pr_err("[update_soc_work]rtc_ui_soc is error:%d ui_soc:%d real_soc:%d\n",rtc_ui_soc ,ui_soc ,real_soc);
        }
        gm->bs_data.bat_capacity = ui_soc;
        g_init_flag = false;
        battery_update(gm);
        schedule_delayed_work(&gm->update_soc_work, msecs_to_jiffies(5000));
        return;
	}
	diff = timespec_sub(now_time, old_time);
	if ((is_done || g_into_warm_over_cv) && ui_soc < 100 && diff.tv_sec > 10){
		ui_soc++;
		old_time = now_time;
		pr_err("[update_soc_work]is_done:%d into_warm_over_cv:%d, uisoc will to 100,ui_soc:%d, real_soc:%d\n", is_done, g_into_warm_over_cv, ui_soc, real_soc);
		if (ui_soc > 100)
			ui_soc = 100;
	}

	ret = charger_dev_get_charging_status(gm->charger_dev, &charging_status);
	gauge_get_property(GAUGE_PROP_BATTERY_VOLTAGE, &bat_volt);
	pr_err("[update_soc_work]bat_volt:%d, old_time.tv_sec:%d, now_time.tv_sec:%d, diff:%d real_soc:%d suspend_flag:%d\n",bat_volt ,old_time.tv_sec ,now_time.tv_sec ,diff.tv_sec ,real_soc ,gm->charger_dev->suspend_flag);
	if (bat_volt < 3380)
		low_batt = true;
	pr_err("[update_soc_work]charging_status:%d, chr_type:%d, real_soc:%d, ui_soc:%d\n", charging_status ,gm->chr_type , real_soc ,ui_soc);
	if ((charging_status == true) && (real_soc > ui_soc)) {
		if(diff.tv_sec > UISOC_ADD) {
			ui_soc++;
			old_time = now_time;
			if (ui_soc > 100)
				ui_soc = 100;
		}
		pr_err("[update_soc_work]---charging,soc add.ui_soc:%d, real_soc:%d diff:%d \n",ui_soc ,real_soc ,diff.tv_sec);
	} else if ((gm->chr_type == POWER_SUPPLY_USB_TYPE_UNKNOWN || gm->charger_dev->suspend_flag == 1 || (g_into_warm_over_cv == false && !charging_status)) && (real_soc < ui_soc)) {
		if (low_batt){
			if(diff.tv_sec > LOW_BATT_UISOC) {
				ui_soc--;
				old_time = now_time;
				pr_err("[update_soc_work]--batt is low,will to zero\n");
			}
		} else if(diff.tv_sec > UISOC_SUB) {
			ui_soc--;
			if(ui_soc == 0 && bat_volt > 3400)
				ui_soc = 1;
			old_time = now_time;
		}
		if (ui_soc < 0)
			ui_soc = 0;
		pr_err("[update_soc_work]---discharging soc sub.ui_soc:%d, real_soc:%d diff:%d\n",ui_soc ,real_soc ,diff.tv_sec);
	}
	gm->bs_data.bat_capacity = ui_soc;
	gauge_set_property(GAUGE_PROP_RTC_UI_SOC, ui_soc);
	rtc_ui_soc = gauge_get_int_property(GAUGE_PROP_RTC_UI_SOC);
	pr_err("[update_soc_work] rtc_ui_soc:%d, ui_soc:%d real_soc:%d\n",rtc_ui_soc ,ui_soc ,real_soc);
	battery_update(gm);
	schedule_delayed_work(&gm->update_soc_work, msecs_to_jiffies(5000));
}
#endif

extern bool mtk_charger_get_jeita_charging_status(void);
static int battery_psy_get_property(struct power_supply *psy,
	enum power_supply_property psp,
	union power_supply_propval *val)
{
	int ret = 0;
	union power_supply_propval wait_temp_status;
	struct mtk_battery *gm;
	...
	chg_master_psy = power_supply_get_by_name("mtk-master-charger");
	if (IS_ERR_OR_NULL(chg_master_psy)) {
		bm_err("%s Couldn't get chg_master_psy\n", __func__);
		return -1;
	}
	ret = power_supply_get_property(chg_master_psy,	POWER_SUPPLY_PROP_WAIT_TEMP_STATUS, &wait_temp_status);
	// if (gm->algo.active == true)
	// 	bs_data->bat_capacity = gm->ui_soc;

	switch (psp) {
	case POWER_SUPPLY_PROP_STATUS:
		bm_err("[%s]:pre bat_status = %d, suspend_flag = %d, wait_temp_status = %d, into_warm_over_cv = %d\n", __func__,
				bs_data->bat_status, gm->charger_dev->suspend_flag, wait_temp_status.intval, g_into_warm_over_cv);
		if (bs_data->bat_status != POWER_SUPPLY_STATUS_DISCHARGING) {
			if (wait_temp_status.intval != 0) {
				val->intval = POWER_SUPPLY_STATUS_CHARGING;
				bm_err("Temperature out of range when start charging, report a fake charging status first!!!\n");
			}
			else {
				charger_dev_is_charging_done(gm->charger_dev, &charging_status);
				if (charging_status || g_into_warm_over_cv) {
					...
					charger_dev_get_charging_status(gm->charger_dev, &charging_status);
					if (charging_status)
						val->intval = POWER_SUPPLY_STATUS_CHARGING;
					else
...
#ifdef CONFIG_BATTERY_GAUGE_MM8013C10
		val->intval = mm8013c10_get_bat_info(MM8013C10_BAT_CC);
		break;
#else
		val->intval = 1;
		break;
	...
	case POWER_SUPPLY_PROP_STOPCHARGING_TEST:
		gm->charger_dev->suspend_flag = 1;
		charger_dev_enable(gm->charger_dev, 0x0);
		bm_err("Application control StopCharging_Test!!!\n");
		val->intval = 0;
		power_supply_changed(chg_psy);
		break;
	case POWER_SUPPLY_PROP_STARTCHARGING_TEST:
		gm->charger_dev->suspend_flag = 0;
		charger_dev_enable(gm->charger_dev, 0x1);
		bm_err("Application control StartCharging_Test!!!\n");
		val->intval = 1;
		power_supply_changed(chg_psy);
		break;
	case POWER_SUPPLY_PROP_RESISTANCE_ID:
#ifdef CONFIG_BATTERY_GAUGE_MM8013C10
		val->intval = mm8013c10_get_bat_info(MM8013C10_BAT_ID);
#endif
		break;
	case POWER_SUPPLY_PROP_MANUFACTURER:
#ifdef CONFIG_BATTERY_GAUGE_MM8013C10
		val->strval = mm8013c10_get_battery_name();
#endif
		break;
...
static void mtk_battery_external_power_changed(struct power_supply *psy)
{
	...
	if (IS_ERR_OR_NULL(chg_psy)) {
		...
		bm_err("online = %d, status = %d, suspend = %d, ret = %d\n", online.intval,
		status.intval, gm->charger_dev->suspend_flag, ret);
		if (!online.intval) {
			bs_data->bat_status = POWER_SUPPLY_STATUS_DISCHARGING;
			gm->charger_dev->suspend_flag = 0;
			bm_err("Charger offline, restore suspend_flag\n");
			g_into_warm_over_cv = false;
		} else {
...
int BattThermistorConverTemp(struct mtk_battery *gm, int Res)
{
	...
	if (Res >= ptable[0].TemperatureR) {
		TBatt_Value = -200;
	} else if (Res <= ptable[16].TemperatureR) {
		TBatt_Value = 600;
	} else {
	...
	int val)
{
	...
	if (old_uisoc != gm->ui_soc) {
		...
		pr_err("[%s] FG_DAEMON_CMD_SET_KERNEL_UISOC = %d %d GM3:%d old:%d diff=%ld\n",
			__func__,
...
//		gm->bs_data.bat_capacity = gm->ui_soc;
		...
		pr_err("[%s] FG_DAEMON_CMD_SET_KERNEL_UISOC = %d %d GM3:%d\n",
			__func__,
...
int battery_psy_init(struct platform_device *pdev)
{
	...
	gauge->gm = gm;
	gm->gauge = gauge;
	gm->charger_dev = get_charger_by_name("primary_chg");
	if (gm->charger_dev)
		bm_err("Found primary charger\n");
	else{
...
#ifdef CONFIG_BATTERY_GAUGE_MM8013C10
	INIT_DELAYED_WORK(&gm->update_soc_work, update_soc_handler);
	schedule_delayed_work(&gm->update_soc_work, msecs_to_jiffies(1000));
#endif
