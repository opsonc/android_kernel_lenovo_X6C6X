 ...
 * Filename:
 * ---------
 *	 s5k4h7yx_kingc_main_mipi_raw.c
 *
 * Project:
 * --------
 *	 ALPS
...
#include "s5k4h7yx_kingc_main_mipi_raw.h"
...
static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = S5K4H7YX_KINGC_MAIN_SENSOR_ID,
...
kal_uint16 otp_4h7_kingc_main_read_cmos_sensor(kal_uint32 addr)
{
...
void otp_4h7_kingc_main_write_cmos_sensor_8(kal_uint16 addr, kal_uint8 para)
{
	write_cmos_sensor_8(addr, para);
}

unsigned char S5K4H7_kingc_main_read_cmos_sensor(u32 addr)
{
	kal_uint16 get_byte = 0;
...
void S5K4H7_kingc_main_write_cmos_sensor(u16 addr, u32 para)
{
	char pusendcmd[3] = {
		(char)(addr >> 8), (char)(addr & 0xFF), (char)(para & 0xFF) };
...
static kal_uint8 s5k4h7yx_kingc_otp_id[37];

unsigned int s5k4h7yx_read_region(struct i2c_client *client, unsigned int addr,
		unsigned char *data, unsigned int size)
{
	switch(addr)
	{
		case 0x00000A04: //0x04 Flag of Basic information & AWB Bit[7:6]: 00:empty;01:valid;1x:invalid.
			*data = s5k4h7yx_kingc_otp_id[0];
			break;
		case 0x00000A09: // for DoCamCalLayoutCheck
			*(kal_uint32*)data = (*(kal_uint32*)data | s5k4h7yx_kingc_otp_id[7]) << 8;
			*(kal_uint32*)data = (*(kal_uint32*)data | s5k4h7yx_kingc_otp_id[6]) << 8;
			*(kal_uint32*)data = (*(kal_uint32*)data | s5k4h7yx_kingc_otp_id[5]) << 0;
			break;
		case 0x00000A0C:  //ModuleVersion
			*(kal_int32*)data = (*(kal_int32*)data | s5k4h7yx_kingc_otp_id[8]) << 8;
			*(kal_int32*)data = *(kal_int32*)data | s5k4h7yx_kingc_otp_id[9];
			break;
		case 0x00000A19:  //AWB R Gr, Gb, B, Data
			*data = s5k4h7yx_kingc_otp_id[21];                   // Bit[7:0] : AWB R Data[9:2]
			*(data+1) = (s5k4h7yx_kingc_otp_id[25] >> 6) & 0x03; // Bit[7:6] : AWB R Data[1:0]
			*(data+2) = s5k4h7yx_kingc_otp_id[22];               // Bit[7:0] : AWB Gr Data[9:2]
			*(data+3) = (s5k4h7yx_kingc_otp_id[25] >> 4) & 0x03; // Bit[5:4] : AWB Gr Data[1:0]
			*(data+4) = s5k4h7yx_kingc_otp_id[23];               // Bit[7:0] : AWB Gb Data[9:2]
			*(data+5) = (s5k4h7yx_kingc_otp_id[25] >> 2) & 0x03; // Bit[3:2] : AWB Gb Data[1:0]
			*(data+6) = s5k4h7yx_kingc_otp_id[24];               // Bit[7:0] : AWB B Data[9:2]
			*(data+7) = s5k4h7yx_kingc_otp_id[25] & 0x03;        // Bit[1:0] : AWB B Data[1:0]
			break;
		case 0x00000A1E:  //AWB golden R Gr, Gb, B, Data
			*data = s5k4h7yx_kingc_otp_id[26];                    // Bit[7:0] : Golden AWB R Data[9:2]
			*(data+1) = (s5k4h7yx_kingc_otp_id[30] >> 6) & 0x03;  // Bit[7:6] :Golden AWB R Data[1:0]
			*(data+2) = s5k4h7yx_kingc_otp_id[27];                // Bit[7:0] : Golden AWB Gr Data[9:2]
			*(data+3) = (s5k4h7yx_kingc_otp_id[30] >> 4) & 0x03;  // Bit[5:4] :Golden AWB Gr Data[1:0]
			*(data+4) = s5k4h7yx_kingc_otp_id[28];                // Bit[7:0] : Golden AWB Gb Data[9:2]
			*(data+5) = (s5k4h7yx_kingc_otp_id[30] >> 2) & 0x03;  // Bit[3:2] :Golden AWB Gb Data[1:0]
			*(data+6) = s5k4h7yx_kingc_otp_id[29];                // Bit[7:0] : Golden AWB B Data[9:2]
			*(data+7) = s5k4h7yx_kingc_otp_id[30] & 0x03;         // Bit[1:0] :Golden AWB B Data[1:0]
			break;
		case 0x00000A24:  //AF
			*data = s5k4h7yx_kingc_otp_id[32];                   //0X40 Flag of AF
			*(data+1) = s5k4h7yx_kingc_otp_id[33];               //Bit[7:0] : infinity Code[9:2]
			*(data+2) = s5k4h7yx_kingc_otp_id[34];               //Bit[7:0] : Macro Code[9:2]
			*(data+3) = (s5k4h7yx_kingc_otp_id[35] >> 6) & 0x03; //Bit[7:6] : infinity Code[1:0]
			*(data+4) = (s5k4h7yx_kingc_otp_id[35] >> 4) & 0x03; //Bit[5:4] : Macro Code[1:0]
			break;
		case 0x00000A06:  //PartNumber
			*data = s5k4h7yx_kingc_otp_id[2];
			*(data+1) = s5k4h7yx_kingc_otp_id[3];
			*(data+2) = s5k4h7yx_kingc_otp_id[4];
			break;
		default:
			*data = 0;
			break;
...
static int s5k4h7yx_kingc_get_otp_data(void)
{
	int i,j;
	int page = 0x15; // page 21
	int AF_flag = 0, AWB_flag = 0;  //AF_flag: Flag of AF, AWB_flag: Flag of Basic information & AWB
	unsigned int calculate_awb_sum = 0;
	unsigned int calculate_af_sum = 0;
	LOG_INF("s5k4h7yx get_otp_data start");
	for(i = 0;i < 3;i++)
	{
		/* OTP Read Sequence */
		write_cmos_sensor_8(0x0100, 0x01); // Streaming On
		mdelay(50);
		write_cmos_sensor_8(0x0A02, page+i); // Write OTP page:0x15 page 21
		write_cmos_sensor_8(0x0A00, 0x01); // Read Commamd

		if(read_cmos_sensor(0x0A01))
		{
			LOG_INF("otp1 s5k4h7 i = %d\n",i);
			AWB_flag = read_cmos_sensor(0x0a04);
			AF_flag = read_cmos_sensor(0x0a24);
			/* GET S5K4H7YX OTP AWB DATA */
			switch(S5K4H7YX_OTP_GET_2BIT_FLAG(AWB_flag, 6))
			{
				case S5K4H7YX_OTP_FLAG_EMPTY:
					LOG_INF("group %d awb data is empty!\n",page + i);
					break;
				case S5K4H7YX_OTP_FLAG_VALID:
					LOG_INF("group %d awb data is valid!\n",page + i);
					for(j = 0;j < 32;j++)
					{
						s5k4h7yx_kingc_otp_id[j] = read_cmos_sensor(0x0a04 + j);
						//LOG_INF("otp s5k4h7 s5k4h7yx_kingc_otp_id[%d] 0x%x",j,s5k4h7yx_kingc_otp_id[j]);
					}
					//calculate awb checksum
					for(j = 1;j < 31;j++)
						calculate_awb_sum += s5k4h7yx_kingc_otp_id[j];

					if((calculate_awb_sum % 255 + 1) == s5k4h7yx_kingc_otp_id[31])
					{
						LOG_INF("S5K4H7YX AWB checksum right!\n");
					}
					else
					{
						LOG_INF("S5K4H7YX AWB checksum error!\n");
					}
					break;
				case S5K4H7YX_OTP_FLAG_INVALID:
					LOG_INF("group %d awb data is invalid type 1 !\n",page + i);
					break;
				case S5K4H7YX_OTP_FLAG_INVALID2:
					LOG_INF("group %d awb data is invalid type 2 !\n",page + i);
					break;
				default:
					break;
			}
			/* GET S5K4H7YX OTP AF DATA */
			switch(S5K4H7YX_OTP_GET_2BIT_FLAG(AF_flag, 6))
			{
				case S5K4H7YX_OTP_FLAG_EMPTY:
					LOG_INF("group %d AF data is empty!\n",page + i);
					break;
				case S5K4H7YX_OTP_FLAG_VALID:
					LOG_INF("group %d AF data is valid!\n",page + i);
					for(j = 32;j < 37;j++)
					{
						s5k4h7yx_kingc_otp_id[j] = read_cmos_sensor(0x0a04 + j);
						LOG_INF("otp s5k4h7 s5k4h7yx_kingc_otp_id[%d] 0x%x",j,s5k4h7yx_kingc_otp_id[j]);
					}
					//calculate AF checksum
					for(j = 33;j < 36;j++)
						calculate_af_sum += s5k4h7yx_kingc_otp_id[j];

					if((calculate_af_sum % 255 + 1) == s5k4h7yx_kingc_otp_id[36])
					{
						LOG_INF("S5K4H7YX AF checksum right!\n");
					}
					else
					{
						LOG_INF("S5K4H7YX AF checksum error!\n");
					}
					break;
				case S5K4H7YX_OTP_FLAG_INVALID:
					LOG_INF("group %d AF data is invalid type 1 !\n",page + i);
					break;
				case S5K4H7YX_OTP_FLAG_INVALID2:
					LOG_INF("group %d AF data is invalid type 2 !\n",page + i);
					break;
				default:
					break;
			}
			if( ((calculate_awb_sum % 255 + 1) == s5k4h7yx_kingc_otp_id[31]) && ((calculate_af_sum % 255 + 1) == s5k4h7yx_kingc_otp_id[36]) )
			{
				LOG_INF("group %d awb&af data is valid!\n",page + i);
				break;
			}
		}
	}
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
	...
	*sensor_id = 0xFFFFFFFF;
	...
	LOG_INF("cxc_main_kingc vendor_id 0x%x\n", vendor_id);
		...
		do {
			//if (0x06 == vendor_id) {
				*sensor_id = return_sensor_id();
				LOG_INF
				    ("cxc_main_kingc sensor id: 0x%x, imgsensor_info.sensor_id 0x%x imgsensor.i2c_write_id: 0x%x\n",
...
UINT32 S5K4H7YX_KINGC_MAIN_MIPI_RAW_SensorInit(struct SENSOR_FUNCTION_STRUCT
						**pfFunc)
{
