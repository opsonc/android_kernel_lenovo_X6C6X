 ...
 * Filename:
 * ---------
 *	 OV8856_qtech_main_mipiraw_Sensor.c
 *
 * Project:
 * --------
 *	 ALPS MT6795
...
#include "ov8856_qtech_main_mipiraw_Sensor.h"
#include <linux/slab.h>

#define PFX "OV8856"
#define OV8856R2AOTP //daizhiling add for OTP
...
struct ov8856_qtech_otp_data_struct {
	char awb_flag;          //flag of basic info&AWB
	char module_house_id;   //0x06: Qtech
	char sensor_id;         //0xDA: OV8856-GA5A
	char awb_data[5];
	char af_data[3];
	int checksum1;         //checksum for group without Flag&LSC
	char lsc_flag;         //flag of LSC
	char lsc_data[240];
	int checksum2;         //checksum for LSC data
	char lens_id;          //0x27: xingjuyu SPY8111C
	char vcm_id;           //0X43: youhua   VV10-12
};

static struct ov8856_qtech_otp_data_struct ov8856_qtech_otp_data = {0};

static imgsensor_info_struct imgsensor_info = {
	.sensor_id = OV8856_QTECH_MAIN_SENSOR_ID,		//record sensor id defined in Kd_imgsensor.h
	...
	.isp_driving_current = ISP_DRIVING_8MA, //mclk driving current  ISP_DRIVING_2MA
		...
		do {
				*sensor_id = return_sensor_id();
				if (*sensor_id == imgsensor_info.sensor_id) {
					LOG_INF("[ov8856_qtech_main]get sensor id:imgsensor.i2c_write_id: 0x%x, *sensor_id=0x%x\n",
		imgsensor.i2c_write_id, *sensor_id);
				return ERROR_NONE;

				}
	LOG_INF("[ov8856_qtech_main]Read sensor id fail,imgsensor.i2c_write_id: 0x%x, *sensor_id=0x%x\n",
		imgsensor.i2c_write_id, *sensor_id);
			retry--;
		} while (retry > 0);
		i++;
...
unsigned int ov8856_read_region(struct i2c_client *client, unsigned int addr, unsigned char *data, unsigned int size){

	int i;
	switch(addr)
	{
		case 0x00007010: //0x04 Basic information & AWB Bit[7:6] flag of group 1: 00:empty;01:valid;1x:invalid.
			*data = ov8856_qtech_otp_data.awb_flag;
			LOG_INF("otp1228 ov8856_read_region addr=0x%x data=0x%x\n",addr,*data);
			break;
		case 0x0000720B: // for DoCamCalLayoutCheck
			*(kal_uint32*)data = (*(kal_uint32*)data | ov8856_qtech_otp_data.vcm_id) << 8;
			*(kal_uint32*)data = (*(kal_uint32*)data | ov8856_qtech_otp_data.lens_id) << 0;
			LOG_INF("otp1228 ov8856_read_region addr=0x%x data=0x%x\n",addr,*(kal_uint32*)data);
			break;
		case 0x00007013: //for AWB R Gr, Gb, B, Data
			*data = ov8856_qtech_otp_data.awb_data[0];                      // Bit[7:0] : AWB R Data[9:2]
			*(data + 1) = (ov8856_qtech_otp_data.awb_data[4] >> 6) & 0x03;  // Bit[7:6] : AWB R Data[1:0]
			*(data + 2) = ov8856_qtech_otp_data.awb_data[1];                // Bit[7:0] : AWB Gr Data[9:2]
			*(data + 3) = (ov8856_qtech_otp_data.awb_data[4] >> 4) & 0x03;  // Bit[5:4] : AWB Gr Data[1:0]
			*(data + 4) = ov8856_qtech_otp_data.awb_data[2];                // Bit[7:0] : AWB Gb Data[9:2]
			*(data + 5) = (ov8856_qtech_otp_data.awb_data[4] >> 2) & 0x03;  // Bit[3:2] : AWB Gb Data[1:0]
			*(data + 6) = ov8856_qtech_otp_data.awb_data[3];                // Bit[7:0] : AWB B Data[9:2]
			*(data + 7) = ov8856_qtech_otp_data.awb_data[4] & 0x03;         // Bit[1:0] : AWB B Data[1:0]
			for(i = 0;i < 8;i++)
				LOG_INF("otp1228 ov8856_read_region addr=0x%x data[%d]=0x%x\n",addr,i, *(data + i));
			break;
		case 0x00007018: //for AF
			*data = ov8856_qtech_otp_data.af_data[0];                       // Bit[7:0] : inf_DAC ratio[9:2]
			*(data + 1) = (ov8856_qtech_otp_data.af_data[2] >> 6) & 0x03;   // Bit[7:6] : inf_DAC ratio[1:0]
			*(data + 2) = ov8856_qtech_otp_data.af_data[1];                 // Bit[7:0] : Mac_DAC ratio[9:2]
			*(data + 3) = (ov8856_qtech_otp_data.af_data[2] >> 4) & 0x03;   // Bit[5:4] : Mac_DAC ratio[1:0]
			for(i = 0;i < 3;i++)
				LOG_INF("otp1228 ov8856_read_region addr=0x%x data[%d]=0x%x\n",addr,i, *(data + i));
			break;
		default:
...
static void ov8856_qtech_get_otp_data(void)
{
	int temp, i;
	unsigned int addr = 0;
	unsigned int calculate_awb_sum = 0;
	unsigned int calculate_lsc_sum = 0;
	//set 0x5002[3] to ?0?
	temp = read_cmos_sensor(0x5001);
	write_cmos_sensor(0x5001, (0x00 & 0x08) | (temp & (~0x08)));
	// read OTP into buffer
	write_cmos_sensor(0x3d84, 0xC0); //enable OTP read mode
	write_cmos_sensor(0x3d88, 0x70); // OTP start address high byte
	write_cmos_sensor(0x3d89, 0x10); // OTP start address low byte
	write_cmos_sensor(0x3d8A, 0x72); // OTP end address high byte
	write_cmos_sensor(0x3d8B, 0x0a); //OTP end address low byte
	write_cmos_sensor(0x3d81, 0x01); // load otp into buffer
	mdelay(10);
	// read Flag of OTP basic information & AWB
	ov8856_qtech_otp_data.awb_flag = read_cmos_sensor(0x7010);
	LOG_INF("otp1228 awb_flag=0x%x\n",ov8856_qtech_otp_data.awb_flag);

	if((ov8856_qtech_otp_data.awb_flag & 0xc0) == 0x40){
		addr = 0x7011; // base address of info group 1
		ov8856_qtech_otp_data.checksum1 = read_cmos_sensor(0x7025);
		LOG_INF("otp1228 group1 checksum1 = 0x%x\n",ov8856_qtech_otp_data.checksum1);
	}
	else if((ov8856_qtech_otp_data.awb_flag & 0x30) == 0x10){
		addr = 0x701B; // base address of info group 2
		ov8856_qtech_otp_data.checksum1 = read_cmos_sensor(0x7026);
		LOG_INF("otp1228 group2 checksum1 = 0x%x\n",ov8856_qtech_otp_data.checksum1);
	}else{
		LOG_INF("otp1228 The OTP basic info&AWB data in group 1 and group 2 is both invalid !\n");
	}
	ov8856_qtech_otp_data.lens_id = read_cmos_sensor(0x0720B); //0x27
	ov8856_qtech_otp_data.vcm_id = read_cmos_sensor(0x0720C);  //0x43
	LOG_INF("otp1228 lens_id=%x\n",ov8856_qtech_otp_data.lens_id);
	LOG_INF("otp1228 vcm_id=%x\n",ov8856_qtech_otp_data.vcm_id);

	if((addr == 0x7011) || (addr == 0x701B)){
		ov8856_qtech_otp_data.module_house_id = read_cmos_sensor(addr);
		ov8856_qtech_otp_data.sensor_id = read_cmos_sensor( addr + 1);

		for(i = 0;i < 5;i++){
			ov8856_qtech_otp_data.awb_data[i] = read_cmos_sensor(addr + 2 + i);
			//LOG_INF("otp1228 awb_data[%d] = 0x%x\n",i,ov8856_qtech_otp_data.awb_data[i]);
		}
		for(i = 0;i < 3;i++){
			ov8856_qtech_otp_data.af_data[i] = read_cmos_sensor(addr + 7 + i);
			//LOG_INF("otp1228 af_data[%d] = 0x%x\n",i,ov8856_qtech_otp_data.af_data[i]);
		}
		for(i = 0;i < 10;i++)
			calculate_awb_sum += read_cmos_sensor(addr + i);

		calculate_awb_sum += ov8856_qtech_otp_data.lens_id;
		calculate_awb_sum += ov8856_qtech_otp_data.vcm_id;

		if(ov8856_qtech_otp_data.checksum1 == ((calculate_awb_sum % 255) + 1)) {
			LOG_INF("otp1228 TODO AWB checksum right!\n");
		}else{
			LOG_INF("otp1228 checksum data error! TODO AWB checksum1 = 0x%x, calculate result = 0x%x\n",
						ov8856_qtech_otp_data.checksum1, (calculate_awb_sum % 255 + 1));
		}
		LOG_INF("otp1228 module_house_id=0x%x\n",ov8856_qtech_otp_data.module_house_id);
		LOG_INF("otp1228 sensor_id=0x%x\n",ov8856_qtech_otp_data.sensor_id);
	}else {
		LOG_INF("otp1228 otp basic info&AWB addr error!\n");
	}

	// read Flag of OTP LSC
	ov8856_qtech_otp_data.lsc_flag = read_cmos_sensor(0x7028);
	LOG_INF("otp1228 awb_flag=0x%x\n",ov8856_qtech_otp_data.lsc_flag);

	if((ov8856_qtech_otp_data.lsc_flag & 0xc0) == 0x40){
		addr = 0x7029; // base address of group 1 LSC
		ov8856_qtech_otp_data.checksum2 = read_cmos_sensor(0x7119);
		LOG_INF("otp1228 group 1 lsc checksum2 = 0x%x\n",ov8856_qtech_otp_data.checksum2);
	}else if((ov8856_qtech_otp_data.lsc_flag & 0x30) == 0x10){
		addr = 0x711A; // base address of group 2 LSC
		ov8856_qtech_otp_data.checksum2 = read_cmos_sensor(0x720A);
		LOG_INF("otp1228 group 2 lsc checksum2 = 0x%x\n",ov8856_qtech_otp_data.checksum2);
	}else{
		LOG_INF("otp1228 The OTP LSC data in group 1 and group 2 is both invalid !\n");
	}

	if((addr == 0x7029) || (addr == 0x711A)){

		for(i = 0; i < 240; i++){
			ov8856_qtech_otp_data.lsc_data[i] = read_cmos_sensor(addr + i);
			calculate_lsc_sum += ov8856_qtech_otp_data.lsc_data[i];
			//LOG_INF("otp1228 lsc_data[%d] = 0x%x\n", i,ov8856_qtech_otp_data.lsc_data[i]);
		}

		if(ov8856_qtech_otp_data.checksum2 == ((calculate_lsc_sum % 255) + 1)) {
			LOG_INF("otp1228 LSC checksum right!\n");
		}else{
			LOG_INF("otp1228 LSC checksum data error! checksum2 = 0x%x, calculate result = 0x%x\n",
						ov8856_qtech_otp_data.checksum2, (calculate_lsc_sum % 255 + 1));
		}
	}else{
		LOG_INF("otp1228 otp LSC addr error!\n");
	}
...
static kal_uint32 open(void)
{
	...
	LOG_INF("PLATFORM:MT8768,MIPI 4LANE\n");
