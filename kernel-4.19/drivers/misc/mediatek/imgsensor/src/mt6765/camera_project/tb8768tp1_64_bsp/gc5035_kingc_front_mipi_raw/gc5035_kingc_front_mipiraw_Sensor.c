...
#include "gc5035_kingc_front_mipiraw_Sensor.h"
...
static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = GC5035_KINGC_FRONT_SENSOR_ID,
...
#endif
static	kal_uint8 gc5035_kingc_otp_id[GC5035_KINGC_OTP_ID_SIZE];
		...
		unsigned char *data, unsigned int size)
{
	switch(addr)
	{
		case 0x00001E00: //0x04 Flag of Basic information & AWB Bit[7:6]: 00:empty;01:valid;1x:invalid.
			*data = gc5035_kingc_otp_id[0];
			break;
		case 0x00001E28: // for DoCamCalLayoutCheck
			*(kal_uint32*)data = (*(kal_uint32*)data | gc5035_kingc_otp_id[7]) << 8;
			*(kal_uint32*)data = (*(kal_uint32*)data | gc5035_kingc_otp_id[6]) << 8;
			*(kal_uint32*)data = (*(kal_uint32*)data | gc5035_kingc_otp_id[5]) << 0;
			break;
		case 0x00001E10: // PartNumber
			*data = gc5035_kingc_otp_id[2];
			*(data+1) = gc5035_kingc_otp_id[3];
			*(data+2) = gc5035_kingc_otp_id[4];
			break;
		case 0x00001E40: //ModuleVersion Manufacture Date
			*(kal_int32*)data = (*(kal_int32*)data | gc5035_kingc_otp_id[9]) << 8;
			*(kal_int32*)data = *(kal_int32*)data | gc5035_kingc_otp_id[8];
			break;
		case 0x00001E68: //for AWB R/G & B/G ratio
			*data = gc5035_kingc_otp_id[13];                      // Bit[7:0] : AWB R/G ratio[9:2]
			*(data + 1) = (gc5035_kingc_otp_id[15] >> 6) & 0x03;  // Bit[7:6] : AWB R/G ratio[1:0]
			*(data + 2) = gc5035_kingc_otp_id[14];                // Bit[7:0] : AWB B/G ratio[9:2]
			*(data + 3) = (gc5035_kingc_otp_id[15] >> 4) & 0x03;  // Bit[5:4] : AWB B/G ratio[1:0]
			*(data + 4) = gc5035_kingc_otp_id[16];                // Bit[7:0] : Golden_AWB R/G ratio[9:2]
			*(data + 5) = (gc5035_kingc_otp_id[18] >> 6) & 0x03;  // Bit[7:6] : Golden_AWB R/G ratio[1:0]
			*(data + 6) = gc5035_kingc_otp_id[17];                // Bit[7:0] : Golden_AWB B/G ratio[9:2]
			*(data + 7) = (gc5035_kingc_otp_id[18] >> 4) & 0x03;  // Bit[5:4] : Golden_AWB B/G ratio[1:0]
			break;
		default:
			*data = 0; break;
	}
...
static void gc5035_kingc_get_otp_data(void)
{
	kal_uint8 i = 0;
	kal_uint8 page_selct = 0;
	kal_uint8 flag = 0;
	kal_uint16 checksum = 0;
	kal_uint16 group_start_offset = GC5035_KINGC_OTP_ID_DATA_OFFSET;

	memset(&gc5035_kingc_otp_id, 0, GC5035_KINGC_OTP_ID_SIZE);

	for (page_selct = 0; page_selct < 3; page_selct++) {

		flag = gc5035_otp_read_byte(group_start_offset);
		...
		case GC5035_OTP_FLAG_VALID: {
			...
			gc5035_otp_read_group(group_start_offset, &gc5035_kingc_otp_id[0], GC5035_KINGC_OTP_ID_SIZE);

			//calculate checksum
			for (i = 1; i < GC5035_KINGC_OTP_ID_SIZE - 2; i++)
				checksum += gc5035_kingc_otp_id[i];

			if ((checksum % 255 + 1) == gc5035_kingc_otp_id[GC5035_KINGC_OTP_ID_SIZE - 1]) {
				cam_pr_debug("checksum right!\n");
			} else
				cam_pr_debug("checksum group %d error! checksum sum = 0x%x, calculate result = 0x%x\n",
					page_selct + 1,
					gc5035_kingc_otp_id[GC5035_KINGC_OTP_ID_SIZE - 1],
					(checksum % 255 + 1));
			break;
		}
		case GC5035_OTP_FLAG_INVALID:{
			cam_pr_debug("group %d is invalid type1!\n", page_selct + 1);
			break;
		}

		case GC5035_OTP_FLAG_INVALID2: {
			cam_pr_debug("group %d is invalid type2!\n", page_selct + 1);
			break;
		}
		default:
			break;
		}
		if( ((flag & 0xc0) == 0x40) && ((checksum % 255 + 1) == gc5035_kingc_otp_id[GC5035_KINGC_OTP_ID_SIZE - 1]) ){
			cam_pr_debug("otp2 flag = 0x%x\n", flag);
			break;
		}else{
			group_start_offset = group_start_offset + 168;
			cam_pr_debug("otp3 group_start_offset = 0x%x\n", group_start_offset);
		}
...
UINT32 GC5035_KINGC_FRONT_MIPI_RAW_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
	cam_pr_debug("E\n");
