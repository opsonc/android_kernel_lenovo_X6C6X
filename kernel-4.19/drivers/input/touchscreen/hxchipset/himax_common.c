...
struct proc_dir_entry *himax_touch_proc_dir_1;
EXPORT_SYMBOL(himax_touch_proc_dir_1);

int g_mmi_refcnt;
EXPORT_SYMBOL(g_mmi_refcnt);

#define HIMAX_PROC_AND_TOUCH_FOLDER "android_touch"
...
#define HIMAX_PROC_LOCKDOWN_INFO_FILE	"lockdown_info"
struct proc_dir_entry *himax_proc_self_test_file;
struct proc_dir_entry *himax_proc_lockdown_info_file;
...
uint8_t HX_SMWP_EN;
#define HIMAX_PROC_GESTURE_MODE_FILE "gesture_mode"
struct proc_dir_entry *himax_proc_gesture_mode_file;
...
static int himax_lockdown_proc_show(struct seq_file *m, void *v);
static int himax_lockdown_proc_open(struct inode *inode, struct file *file);
static ssize_t himax_lockdown_proc_write(struct file *file, const char __user *buffer, size_t count, loff_t *f_pos);
...
static int himax_lockdown_proc_open(struct inode *inode, struct file *file)
{
	return single_open(file, himax_lockdown_proc_show, NULL);
}
static ssize_t himax_lockdown_proc_write(struct file *file, const char __user *buffer, size_t count, loff_t *f_pos)
{
...
ssize_t himax_lockdown_proc_read(struct file *file, char __user *buf, size_t len, loff_t *off)
{
	int len_copy, ret;
	char data[32] = "0x32";
...
static struct file_operations himax_lockdown_proc_fops = {
	.owner	= THIS_MODULE,
	.open	= himax_lockdown_proc_open,
	.release = single_release,
	.read	= himax_lockdown_proc_read,
	.llseek = seq_lseek,
	.write	= himax_lockdown_proc_write,
};
...
static ssize_t himax_gesture_mode_write(struct file *file,const char __user *buffer,
...
static const struct file_operations himax_proc_gesture_mode_ops = {
	.owner = THIS_MODULE,
	.write = himax_gesture_mode_write,
};
...
int himax_common_proc_init(void)
{
	himax_touch_proc_dir = proc_mkdir(HIMAX_PROC_TOUCH_FOLDER, NULL);
        himax_touch_proc_dir_1 = proc_mkdir(HIMAX_PROC_AND_TOUCH_FOLDER, NULL);
	if (himax_touch_proc_dir == NULL) {
		E(" %s: himax_touch_proc_dir file create failed!\n", __func__);
		return -ENOMEM;
//	himax_touch_proc_dir_1 = proc_mkdir(HIMAX_PROC_AND_TOUCH_FOLDER, NULL);

	//if (himax_touch_proc_di_1 == NULL) {
	//	E(" %s: himax_and_touch_proc_dir file create failed!\n", __func__);
	//	return -ENOMEM;
	}

	if (himax_touch_proc_dir_1 == NULL) {
		E(" %s: himax_and_touch_proc_dir file create failed!\n", __func__);
		return -ENOMEM;
	}
#if defined(CONFIG_TOUCHSCREEN_HIMAX_INSPECT)
	...
	himax_proc_lockdown_info_file = proc_create( HIMAX_PROC_LOCKDOWN_INFO_FILE, 0777,  himax_touch_proc_dir, &himax_lockdown_proc_fops);
	if (!himax_proc_lockdown_info_file) {
	...
	himax_proc_gesture_mode_file = proc_create(HIMAX_PROC_GESTURE_MODE_FILE, 0666,
			himax_touch_proc_dir, &himax_proc_gesture_mode_ops);

	if (himax_proc_gesture_mode_file == NULL) {
		E(" %s: proc GESTURE_MODE file create failed!\n", __func__);
		goto fail_4;
	}
#if defined(HX_ULTRA_LOW_POWER)
...
fail_5:
	...
	remove_proc_entry(HIMAX_PROC_GESTURE_MODE_FILE, himax_touch_proc_dir);
fail_4:
...
{
	remove_proc_entry(HIMAX_PROC_SELF_TEST_FILE, himax_touch_proc_dir);
	remove_proc_entry(HIMAX_PROC_LOCKDOWN_INFO_FILE, himax_touch_proc_dir);
#if defined(HX_HIGH_SENSE)
	remove_proc_entry(HIMAX_PROC_HSEN_FILE, himax_touch_proc_dir);
#endif
#if defined(HX_SMART_WAKEUP)
...
static int i_get_FW(void)
{
	int ret = -1;
	int result = NO_ERR;
	if(panel_info == PANEL_BOE){
		I("%s: file name = %s\n", __func__, BOOT_UPGRADE_FWNAME_BOE);
		ret = request_firmware(&hxfw, BOOT_UPGRADE_FWNAME_BOE, private_ts->dev);
	}else if(panel_info == PANEL_INX){
		I("%s: file name = %s\n", __func__, BOOT_UPGRADE_FWNAME_INX);
...
{
    if(NULL==g_core_fp.fp_chip_init) return 0;
	if (ts->suspended) {
